...
const loadStoredTasks = () => {
    ...
    try {
        ...
        console.warn(
            "Không thể đọc dữ liệu todo_tasks từ localStorage:",
            error
        );
    }
    return fallbackTasks();
...
function TodoApp() {
        ...
        setTasks((prev) => {
            const updated = prev.map((task) => {
                if (
                    ...
                    return { ...task, task_complete: true };
                }
                return task;
            });
    ...
    const filteredTasks = useMemo(() => {
        return tasks
            .filter((task) => {
                if (filter === "active")
                    return !task.task_complete;
                if (filter === "completed")
                    return task.task_complete;
                if (filter === "high")
                    return task.task_prior === 2;
                return true;
            })
            .filter((task) => {
    ...
    const handleAddTask = (data) => {
        setTasks((prev) => {
            const existingTimes = new Set(
                prev.map((task) => task.task_create_time)
            );
            let timestamp =
    ...
    const handleGenerateRandomTask = () => {
        ...
        handleAddTask({
            task_name: capitalize(
                lorem.generateWords(randomBetween(3, 5))
            ),
    ...
    const handleClearCompleted = () => {
        setTasks((prev) =>
            prev.filter((task) => !task.task_complete)
        );
    };
